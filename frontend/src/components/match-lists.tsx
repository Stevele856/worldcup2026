        ...
        <>
            <ul className="mt-4 flex flex-col gap-3">
                {matches.slice(0, visibleCount).map((match,index) =>{
                    ...
                    <li 
                        key={match.id}
                        className="flex animate-in items-center justify-between rounded-lg border border-zinc-200 px-4 py-3 fade-in slide-in-from-bottom-4 fill-mode-both duration-500"
                        style={{animationDelay: `${(index % LIST)*60}ms`}}
                    >
